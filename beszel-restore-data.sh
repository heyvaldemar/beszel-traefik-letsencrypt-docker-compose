#!/usr/bin/env bash
# beszel-restore-data.sh [backup-file-name]
#
# Replaces Beszel's application data with one of the archives the backups
# service wrote.
#
#   ./beszel-restore-data.sh               list and ask
#   ./beszel-restore-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than beszel.
set -Eeuo pipefail
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Beszel." >&2
  exit 1
fi
