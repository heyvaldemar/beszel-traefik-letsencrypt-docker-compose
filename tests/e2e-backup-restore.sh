#!/bin/bash
# End-to-end tests for the beszel-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# The hub keeps everything it knows in one directory: a PocketBase SQLite file
# holding the systems, the alert rules, the users and the metric history. The
# history rides along and nobody restores a CPU graph from March; the alert
# rules are the part that is genuinely irreplaceable. There is no database
# server to dump, so the archive IS the database, and the test that matters is
# whether a file written before a backup comes back after a restore that has
# since destroyed it.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-beszel}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-beszel-traefik-letsencrypt-docker-compose.yml}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq beszel | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: beszel container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^BESZEL_HOSTNAME=.' .env || { echo "  BESZEL_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_data_directory() {
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^beszel_data/'" || { echo "  $f does not carry the data directory" >&2; return 1; }
  echo "  the archive carries the data directory"
}

test_backup_carries_the_database() {
  # A tarball of /beszel_data that happens to contain a directory proves very
  # little. data.db is the instance: the systems, the alert thresholds, the
  # users. An archive missing it restores into a hub that starts perfectly and
  # is monitoring nothing.
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^beszel_data/data.db'" \
    || { echo "  $f does not carry data.db" >&2; return 1; }
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./beszel-restore-data.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./beszel-restore-data.sh failed" >&2; return 1; }
